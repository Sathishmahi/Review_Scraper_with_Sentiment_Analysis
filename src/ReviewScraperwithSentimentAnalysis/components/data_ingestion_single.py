...
from ReviewScraperwithSentimentAnalysis.constant import COLUMNS_NAME,EXTRACT_PRODUCT_COLUMNS_NAME
...
def toExtractImage_etc(html_con):
    ...
    image_urls_list,model_name_list,model_details_list,over_all_reviews_list,all_product_cost_list,all_product_offer_list,free_delivery_list=[],[],[],[],[],[],[]
    over_all_product_content=html_con.findAll(ToExtractImageEtcConstat.OVER_ALL_ELE_TYPE,{"class":ToExtractImageEtcConstat.OVER_ALL_CLASS})

    for con in tqdm(over_all_product_content,desc="to extract info in html_content"):

        cost=con.findAll(ToExtractImageEtcConstat.PRODUCT_PRICE_ELE_TYPE,{"class":ToExtractImageEtcConstat.PRODUCT_PRICE_CLASS})
        offer=con.findAll(ToExtractImageEtcConstat.PRODUCT_OFFER_ELE_TYPE,{"class":ToExtractImageEtcConstat.PRODUCT_OFFER_CLASS})
        model_name=con.findAll(ToExtractImageEtcConstat.MODEL_NAME_ELE_TYPE,{"class":ToExtractImageEtcConstat.MODEL_NAME_CLASS})
        over_all_review=con.findAll(ToExtractImageEtcConstat.OVER_ALL_REVIEW_ELE_TYPE,{"class":ToExtractImageEtcConstat.OVER_ALL_REVIEWS_CLASS})
        image=con.findAll(ToExtractImageEtcConstat.PRODUCT_IMAGE_ELE_TYPE,{"class":ToExtractImageEtcConstat.PRODUCT_IMAGE_CLASS})
        free_delivery=con.findAll(ToExtractImageEtcConstat.FREE_DELIVERY_ELE_TYPE,{"class":ToExtractImageEtcConstat.FREE_DELIVERY_CLASS})
        product_details=con.findAll(ToExtractImageEtcConstat.MODEL_DETAILS__ELE_TYPE,{"class":ToExtractImageEtcConstat.MODEL_DETAILS_CLASS})
        
        over_all_product_details=None
        if product_details:
            over_all_product_details=' '.join([detail.text for detail in product_details])
        ...
        over_all_reviews_list.append(costModelNameReviewFreeDeliveryFun(over_all_review))
