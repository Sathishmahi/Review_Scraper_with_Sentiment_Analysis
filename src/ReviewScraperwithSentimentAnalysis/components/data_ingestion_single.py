...
def to_save_img(all_img_links: list[str], image_name_list: list[str],remove_prevoius_image: bool= True):
    """
    ...
    if remove_prevoius_image and len(all_pre_imgs_li)>0:
        nothing = [os.remove(os.path.join(extract_image_dir_name,im)) for im in all_pre_imgs_li]
        del nothing
    try:
...
def toExtractReviewsSingle(searchString: str, configuration=Configuration()) -> None:
    try:
        ...
        logging.info(f" all product details stored into {extract_product_csv_file_path} ")
