...
class Training:
    ...
    def to_split_train_evaluation(self,df:pd.DataFrame,out_column_name:str)->tuple:
        eval_per=self.training_config.eval_data_per
        x_train,x_test,y_train,y_test=train_test_split(df.iloc[:,0],df[out_column_name],test_size=eval_per,random_state=100)
    ...
    def apply_buffer_and_batch_size(self,x_train,x_test,y_train,y_test):
        ...
        train_data=tf.data.Dataset.from_tensor_slices((x_train,y_train)).shuffle(buffer_size).batch(batch_size).prefetch(tf.data.experimental.AUTOTUNE)
        eval_data=tf.data.Dataset.from_tensor_slices((x_test,y_test)).batch(batch_size).prefetch(tf.data.experimental.AUTOTUNE)
    ...
    def model_training(self,train_data):
        ...
        tf.keras.layers.Bidirectional(
            tf.keras.layers.LSTM(units=birnn_units)),
    ...
    def combine_all(self):
        ...
        x_train,x_test,y_train,y_test=self.to_split_train_evaluation(df,self.training_config.output_columns_name)
        train_data,test_data=self.apply_buffer_and_batch_size(x_train, x_test, y_train, y_test)
